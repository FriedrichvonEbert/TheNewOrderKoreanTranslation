...
**1. 번역 이전**
  ...
  - 따로 인터페이스 번역을 할 것이 아니라면, 반드시 `localisation` 폴더의 파일만 번역합니다. interface 부분 번역이 필요할 경우 번역팀 디스코드의 TNO 탭에서 먼저 파일의 번역 가능 여부를 물어봐주세요.
  - 번역할 파일을 골라서 번역을 하기 전에, Issues 탭에 들어가서 번역을 할 파일을 미리 말해두기 바랍니다. 같은 부분을 동시에 번역해 conflict가 나는 불상사가 없도록 합시다.
...
```
  - 번역할 시 Himmler는 반드시 힘러라고 번역합니다. 또한 러시아 인명이나 지명에 대해서 끝이 -v로 끝나면 보통은 -프라고 음차하시면 됩니다 (예: Malenkov -> 말렌코프)
